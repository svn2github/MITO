...
//static const int FOCUS_TIMER_TICKS = 1000; // Mod. A. Placitelli

const wxChar* wxTreeListCtrlNameStr = _T("treelistctrl");

static wxTreeListColumnInfo wxInvalidTreeListColumnInfo;

int MyDumbDateComparator(const wxDateTime &date1, const wxDateTime &date2)
{
...
public:
	...
	void OnSize( wxSizeEvent &event );	// Modifica Alessio Placitelli
...
public:
	...
	void *GetItemDataPtr(const wxTreeItemId& item) const;	// Modifica Alessio Placitelli
    ...
    void SetItemDataPtr(const wxTreeItemId& item, void *pDataPtr);	// Modifica Alessio Placitelli
...
protected:
    ...
    wxEditTextCtrl      *m_renameControl;
...
protected:
    ...
    bool SendEvent(wxEventType event_type, wxTreeListItem *item, const wxPoint &p = wxDefaultPosition, bool veto = false);  // returns true if processed  --TODO: redesign to always use SendEvent()
...
public:
	...
	void *GetDataPtr() const {return m_pCustomData; }	// Modifica Alessio Placitelli
	...
	void SetDataPtr( void *pDataPtr ) {m_pCustomData = pDataPtr; }	// Modifica Alessio Placitelli
...
private:
	...
	void			   *m_pCustomData;	// Modifica Alessio Placitelli (Scazziamo il padding :) )
...
void wxEditTextCtrl::OnChar( wxKeyEvent &event )
{
    ...
    if (event.GetKeyCode() == WXK_RETURN)
    {
        ...
        if ((*m_res) != m_startValue)
            if (m_owner) m_owner->OnRenameAccept(false);  // not cancelled

        if (!wxPendingDelete.Member(this))
            wxPendingDelete.Append(this);

        m_finished = true;
        if (m_owner) m_owner->SetFocus(); // This doesn't work. TODO.
...
BEGIN_EVENT_TABLE(wxTreeListHeaderWindow,wxWindow)
	...
	EVT_SIZE		  (wxTreeListHeaderWindow::OnSize)	// Modifica Alessio Placitelli
END_EVENT_TABLE()
...
void wxTreeListHeaderWindow::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
    ...
    for ( int i = 0; i < numColumns && x < w; i++ )
    {
		...
		// Modifica Alessio Placitelli
		wxRendererNative::Get().DrawHeaderButton(this, dc, rect, flags, column.GetSortStatus(), &params);
    }
...
// Modifica Alessio Placitelli
void wxTreeListHeaderWindow::OnSize ( wxSizeEvent & event) 
{
	// Gestiamo il ridimensionamento della lista in questo modo	:
	// 1 - Calcoliamo in che proporzione sono le varie colonne rispetto alla
	//     grandezza originaria
	// 2 - Calcoliamo le nuove dimensioni delle colonne, rispettando
	//     le proporzioni prima calcolate
	...
	int iClientWidth = GetWidth();
	int iNewWidth = event.GetSize().GetWidth();

	for(int k = 0; k < iColCount; k++)
	{
		float fColRatio = ((float)GetColumnWidth(k)) / ((float)iClientWidth);
		int iNewColSize = floor(fColRatio * iNewWidth);
		if(iNewColSize <= 0)
		{
			// Colonne degeneri..
			iNewColSize = 1;
...
wxTreeListItem::wxTreeListItem (wxTreeListMainWindow *owner,
    ...
    m_data = data;
	m_pCustomData = NULL;	// Modifica Alessio Placitelli
    m_x = 0;
    m_y = 0;
    m_text_x = 0;
...
wxTreeListItem::~wxTreeListItem() {
	...
	m_pCustomData = NULL;	// Modifica Alessio Placitelli NB: Qui non vogliamo de allocare la struttura dati puntata!
...
BEGIN_EVENT_TABLE(wxTreeListMainWindow, wxScrolledWindow)
	...
	//EVT_TIMER		   (-1, wxTreeListMainWindow::OnFocusTimer)
END_EVENT_TABLE()
...
void wxTreeListMainWindow::Init() {
    ...
    m_renameControl = (wxEditTextCtrl *)NULL;
...
wxTreeListMainWindow::~wxTreeListMainWindow() {
    ...
    if (m_renameControl) delete m_renameControl;
...
// Modifica Alessio Placitelli
void *wxTreeListMainWindow::GetItemDataPtr (const wxTreeItemId& item) const 
{
    wxCHECK_MSG (item.IsOk(), NULL, _T("invalid tree item"));
    return ((wxTreeListItem*) item.m_pItem)->GetDataPtr();
}
...
// Modifica Alessio Placitelli
void wxTreeListMainWindow::SetItemDataPtr (const wxTreeItemId& item, void *pDataPtr )
{
    wxCHECK_RET (item.IsOk(), _T("invalid tree item"));
    ((wxTreeListItem*) item.m_pItem)->SetDataPtr(pDataPtr);
}
...
void wxTreeListMainWindow::Expand (const wxTreeItemId& itemId) {
    ...
    // NOTE: can't used SendEvent has we need to check event state afterwards
    ...
    event.SetInt (m_curColumn); // the colum clicked
...
bool wxTreeListMainWindow::SelectItem (const wxTreeItemId& itemId,
    ...
    // NOTE: not using SendEvent() as we need to check event afterwards
...
void wxTreeListMainWindow::OnChar (wxKeyEvent &event) {
    // send event to user code
    // NOTE: we don't use SendEvent() because this one needs SetKeyEvent()
...
void wxTreeListMainWindow::EditLabel (const wxTreeItemId& item, int column) {
    ...
    // NOTE: we can't use SendEvent() because we need to check the event's state afterwards
    ...
    m_owner->GetEventHandler()->ProcessEvent (te); if (!te.IsAllowed()) return;
    ...
    m_renameControl = new wxEditTextCtrl (this, -1, &m_renameAccept, &m_renameRes,
                                               this, m_editItem->GetText (column),
                                               wxPoint (x, y), wxSize (w, h), style);
    m_renameControl->SetFocus();
}
...
/*void wxTreeListMainWindow::OnFocusTimer(wxTimerEvent& event)
{
	SetFocus();
}*/
...
void wxTreeListMainWindow::OnRenameAccept(bool isCancelled) {
    ...
    // NOTE: we can't use SendEvent() because we need to check the event's state afterwards (+ call to SetEditCanceled())
    ...
    le.SetInt(GetMainColumn());  // label column
...
bool bSkip = true;
	...
	// A. Placitelli: Gestiamo l'autofocus del controllo con
	// un timer
	/*if(event.Entering())
	{
		m_focusTimer->Start (FOCUS_TIMER_TICKS, wxTIMER_ONE_SHOT);
	...
	// Modifica Alessio Placitelli 
	// Settiamo il focus sul controllo se il mouse ci passa sopra..
	//SetFocus();
...
// ----------  HANDLE DRAGGING  ----------
// NOTE: drag is only with left mouse button
// (no changes to selection)
    if (mayDrag) {  // actually this is always true
        ...
        if (m_isDragging) {
            ...
            if (event.LeftDown() || event.MiddleDown() || event.RightDown()) {
                ...
                // middle & right-click may be processed further below
                bSkip = false;
            ...
            // CASE 1.3: dragging now ends normally
                ...
                // send drag end event
                SendEvent(wxEVT_COMMAND_TREE_END_DRAG, item, p);

                bSkip = false;
            }

        // CASE 2: not were not dragging => no change, start
        } else if (event.Dragging()){
            ...
            SendEvent(command, item, p, true);

            bSkip = false;

        } else {
            // just in case dragging had been initiated
            if (m_dragCount > 0) {
                m_dragCount = 0;  // end dragging
            }
        }
    }
...
// HANDLE SIMPLE-CLICKS (selection change, contextual menu)
    if (mayClick) {
        ...
        // normally left and right click change selection
        // NOTE: unlike Windows, for right-click we don't care if CTRL or SHIFT is pressed
        if (maySelect && (event.LeftDown() || event.RightDown())) {

            // how is selection altered
            // keep or discard already selected ?
            m_lastOnSame = (item == m_curItem);
            bool unselect_others =
                event.RightDown()  // a right-click always unselect all others
             || ! (HasFlag(wxTR_MULTIPLE) && (event.ShiftDown() || event.ControlDown()));
        ...
        if (event.RightDown()) {
            ...
            if (item) SendEvent(wxEVT_COMMAND_TREE_ITEM_MENU, item, p);
        }
...
int wxTreeListMainWindow::GetBestColumnWidth (int column, wxTreeItemId parent) {
	...
	// A.Placitelli: evitiamo che scompaiano le colonne se l'album è vuoto
	return (width) ? width : 15;
}


bool wxTreeListMainWindow::SendEvent(wxEventType event_type, wxTreeListItem *item, const wxPoint &p, bool veto) {
    wxTreeEvent nevent (event_type, m_owner->GetId());
    nevent.SetEventObject (m_owner);
    nevent.SetInt (m_curColumn); // the mouse colum
    nevent.SetPoint (p);  // only meaningfull for drag & contextual menu
#if !wxCHECK_VERSION(2, 5, 0)
    ...
    if (veto) nevent.Veto(); // must then be explicit allowed!

    return m_owner->GetEventHandler()->ProcessEvent (nevent);
}
...
BEGIN_EVENT_TABLE(wxTreeListCtrl, wxControl)
	...
	EVT_LIST_COL_CLICK(wxID_ANY, wxTreeListCtrl::OnColClicked)
END_EVENT_TABLE();
...
void wxTreeListCtrl::OnColClicked(wxListEvent& event)
{
	// Modifica Alessio Placitelli
	// Effettuiamo l'ordinamento degli studi!
	m_iColumnToSort = event.GetColumn();
	if(m_iColumnToSort == -1)
		return;

	int countCol = GetColumnCount();

	wxTreeListColumnInfo tmpCol = GetColumn(m_iColumnToSort);
	switch(tmpCol.GetSortStatus())
	{
	case wxHDR_SORT_ICON_UP:
		tmpCol.SetSortStatus(wxHDR_SORT_ICON_DOWN);
		break;
	case wxHDR_SORT_ICON_DOWN:
		tmpCol.SetSortStatus(wxHDR_SORT_ICON_NONE);
		break;
	default:
		tmpCol.SetSortStatus(wxHDR_SORT_ICON_UP);
	}
	SetColumn(m_iColumnToSort, tmpCol);

	// Eliminiamo lo stile con la "freccetta" dalle altre colonne
	for (int iCol = 0; iCol < countCol; iCol++) 
	{

		tmpCol = GetColumn(iCol);
		if(m_iColumnToSort != iCol)
		{
			tmpCol.SetSortStatus(wxHDR_SORT_ICON_NONE);
			SetColumn(iCol, tmpCol);
		}
	}
	...
	// Ricoloriamo le righe..
	DoAlternateRowColors();
}
...
// Modifica Alessio Placitelli
void* wxTreeListCtrl::GetItemDataPtr(const wxTreeItemId& item) const
{ return m_main_win->GetItemDataPtr(item); }
...
// Modifica Alessio Placitelli
void wxTreeListCtrl::SetItemDataPtr(const wxTreeItemId& item,
                                 void* data)
{ m_main_win->SetItemDataPtr(item, data); }
                                   ...
                                   const wxTreeItemId& item2)
{
	...
	// In realtà avere tutte le logiche di sorting hardcoded in questa
	// funzione è una forzatura, ma... 
	wxTreeListColumnInfo colToSort = GetColumn(m_iColumnToSort);
	if(colToSort.GetSortStatus() == wxHDR_SORT_ICON_NONE)
		return 0;	// forse -1?

	int iAscending = (colToSort.GetSortStatus() == wxHDR_SORT_ICON_UP) ? 1 : -1; 

	// Verifichiamo se stiamo ordinando una data
	if(m_iColumnToSort != -1)
	{
			wxString strField1 = GetItemText(item1, m_iColumnToSort);
			wxString strField2 = GetItemText(item2, m_iColumnToSort);

			// Sorting interi: verifichiamo che i due campi siano interi, così:
			//	- Estraiamo gli interi dalla string
			//	- Riconvertiamo gli interi in stringa
			//  - Se il risultato della conversione a stringa è uguale alla stringa
			//	  iniziale allora il nostro campo è un intero ;)
			int iField1 = wxAtoi(strField1);
			int iField2 = wxAtoi(strField2);

			wxString sTmp1 = wxString::Format(wxT("%i"), iField1);
			wxString sTmp2 = wxString::Format(wxT("%i"), iField2);
			if((sTmp1 == strField1) && (sTmp2 == strField2))
			{
				int retVal = 0;

				if(iField1 < iField2)
					retVal = -1;
				else if(iField1 == iField2)
					retVal = 0;
				else if(iField1 > iField2)
					retVal = 1;

                return retVal * iAscending;
			}

			// Se il pacs invia date non valide, ParseDate ritorna NULL
			// ed i metodi di wxDateTime chiamati dopo lanciano (giustamente) 
			// una eccezzione. Per evitare tutto ciò, usciamo dallo switch
			// case con il break in modo da eseguire un semplice controllo
			// strcmp..
			wxDateTime date1, date2;

			if(date1.ParseDate(strField1) && date2.ParseDate(strField2))
			{
				int retVal = 0;

				if(date1.IsEarlierThan(date2))
					retVal = -1;
				else if(date1.IsSameDate(date2))
					retVal = 0;
				else if(date1.IsLaterThan(date2))
					retVal = 1;
	...
	case 1:	// Birthdate
		{
			...
			// case con il break in modo da eseguire un semplice controllo
			// strcmp..
			wxString tmp = GetItemText(item1, m_iColumnToSort);
			if(!date1.ParseDate(tmp))
				break;	
			
			tmp = GetItemText(item2, m_iColumnToSort);
			if(!date2.ParseDate(tmp))
				break;
	...
	case 3:	// Study Date
		{
			...
			wxString tmp2 = GetItemText(item2, m_iColumnToSort);
			if(!date2.ParseDate(tmp2))
				break;
...
*/
	return wxStrcmp(GetItemText(item1, m_iColumnToSort), GetItemText(item2, m_iColumnToSort)) * iAscending;
}
...
void wxTreeListCtrl::DoAlternateRowColors(wxColor c1, wxColor c2)
{
	...
	wxTreeItemIdValue cookie;
	wxTreeItemId subItem = GetFirstChild(GetRootItem(), cookie);

	while(subItem)
	{
		// Alterniamo il colore dei records..
		SetItemBackgroundColour(subItem, (bAlternate) ? c2 : c1);

		subItem = GetNextChild (GetRootItem(), cookie);
